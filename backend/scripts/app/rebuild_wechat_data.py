...
def main():
    ...
    df["is_known_label"] = df["sentiment_label"].isin(["Positive", "Negative", "Neutral"]).astype(int)

    print("5) 生成额外分析列 ...")
    df["sentiment_abs"] = df["sentiment_score"].abs()
    df["sentiment_direction"] = df["sentiment_score"].apply(
        lambda x: "Positive" if pd.notna(x) and x > 0 else ("Negative" if pd.notna(x) and x < 0 else ("Neutral" if pd.notna(x) and x == 0 else "Unknown"))
    )
